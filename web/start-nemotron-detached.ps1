# PowerShell script to start Nemotron Docker container in detached mode
# This avoids the TTY issue in Windows terminals
...
# Start the Docker container in detached mode (no -it flags)
Write-Host "Starting Docker container in detached mode..." -ForegroundColor Yellow
Write-Host "Note: This will download a large image if not already present. This may take several minutes." -ForegroundColor Cyan
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "`nFailed to start container. Check Docker logs for details." -ForegroundColor Red
}
